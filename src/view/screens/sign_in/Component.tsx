...
const SignIn: React.FC<Props> = (props: Props) => {
  ...
  return (
    ...
    <Text style={tw`text-left w-full block text-sm font-medium text-gray-700`}>Password</Text>
      ...
      />
      <TouchableOpacity onPress={signIn} style={tw`w-full rounded-xl bg-blue-500 mt-2 text-center`}>
        <Text style={tw`text-center py-3 text-white font-semibold`}>Login</Text>
        </TouchableOpacity>
        <Text style={tw`mt-3`}  onPress={navigateToSignUp}>Don't have an account? Sign up now</Text>
        </View>
